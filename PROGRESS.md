...
| 9 | Button | [`figma-scripts/09-button.js`](figma-scripts/09-button.js) | Icon/Spinner/Label, sizes built from real padding tokens rather than hardcoded heights (reproduces shadcn's documented h-9/h-8/h-10 exactly). Variant `Style`: Default/Destructive/Outline/Secondary/Ghost/Link. Variant `Size`: Default/Sm/Lg/Icon (24 variants total). Booleans: `Has Icon` (placeholder chip, not bound on Icon size — always visible there), `Loading` (a real ring via `EllipseNode.arcData` — Figma's `vectorPaths` doesn't support SVG arc commands, which is what first broke this), `Disabled` (visibility-bound scrim). Flagged: some sources mention additional icon-xs/icon-sm/icon-lg sizes that couldn't be confirmed against live docs — not built; ask if you want them added. |
